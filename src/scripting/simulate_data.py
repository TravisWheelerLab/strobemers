...
def create_correlated_data(query_file, reference_file, experiment_name,
        seq_len, edit_distance_range):
    directory = os.path.join("data/inputs", experiment_name)
    seq1 = "".join([random.choice("ACGT") for i in range(seq_len)])
    os.mkdir(directory)
    ...
    with open(os.path.join(directory, reference_file), 'w') as o:
        ...
        for edit_distance in edit_distance_range:
            ...
            o.write(f">query1+{current_distance} | len_{len(seq2)} +{edit_distance} \n{''.join(seq2)}\n")
...
def experiment2():
    create_correlated_data("query.fasta", "references.fasta", "experiment2", 10000, range(0, 5000, 10))

def experiment3():
    create_correlated_data("query.fasta", "references.fasta", "experiment3", 1000, range(0, 600, 1))
